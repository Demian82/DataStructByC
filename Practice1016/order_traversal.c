...
void tree_pre_order(treeType* tree)
{
	...
	{
		// 방문(visit)은 스택에서 꺼낸(pop) 노드로부터 출발
		// 꺼낸 트리 노드를 부모 노드로 지정
		tree->parent = linked_stack_pop(stack);

		printf("%c", tree->parent->data);  // visit
...
void linked_tree_menu(treeType* tree)
{
	...
	while (1)
	{
		printf("Linked Binary Tree 1.Add, 2.Level Order, 3.Pre Order "
			"4.In Order, 5.Display, 6.Exit\n");
