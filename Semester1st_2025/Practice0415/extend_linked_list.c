...
void linked_list_delete(listType* list)
{
	node* nptr = NULL; // 삭제 포인터
	node* pptr = NULL; // nptr의 이전 노드 포인터
	int delitem, index, pos;

	// 첫 번쨰 노드 삭제
	nptr = list->head;
	list->head = list->head->link; // nptr->link;
	delitem = nptr->data; // 삭제 데이터 기록
	free(nptr); // 노드 삭제
	list->length--;
	...
	// 마지막 노드 삭제
	nptr = list->head;
	// 첫 번째 노드를 가리킨 후 마지막 노드로 이동
	
	
	// 노드가 1개만 존재하는 경우
	if (nptr->link == NULL) // list->length == 1
	{
		list->head = NULL; // 헤더가 NULL을 가리킴
	}
	else // 노드가 2개 이상인 경우
	{
		/* while (nptr->link != NULL) // 다음 노드가 있으면 이동
	    {
		nptr = nptr->link;
	    }*/
		for (index = 1; index < list->length; index++)
		{
			pptr = nptr; // 이동하기 전에 포인터를 지정하고
			nptr = nptr->link; // 이동
		}
		pptr->link = nptr->link; // NULL;
	}
	...
	// 중간(3번쨰)의 노드를 삭제하는 경우
	pos = 3;
	nptr = list->head;
	for (index = 1; index < pos; index++) // pos 위치로 포인터 이동
	{
		pptr = nptr; // nptr의 이전 포인터
		nptr = nptr->link;
	}
	pptr->link = nptr->link;
